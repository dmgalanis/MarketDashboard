...
def polygon_get(path, params, api_key):
    """Make a GET request to the Polygon REST API."""
    params["apiKey"] = api_key
    url = f"{POLYGON_BASE}{path}?{urlencode(params)}"
    req = Request(url, headers={"Accept": "application/json"})
    with urlopen(req, timeout=30) as resp:
        return json.loads(resp.read().decode())


def fetch_polygon_sector_map(api_key):
    """Fetch all active US common stock tickers from Polygon and return a
    dict mapping ticker → sector string.

    Uses /v3/reference/tickers with market=stocks, type=CS, active=true,
    limit=1000, paginating via next_url until exhausted.
    Sector is derived from sic_code via sic_to_sector().
    Falls back to empty dict on any error so the rest of the job continues.
    """
    if not api_key:
    ...
    print("  [Polygon] Fetching sector map via /v3/reference/tickers ...")
    try:
        while True:
            page += 1
            if next_url:
                # next_url already includes apiKey; just fetch it directly
                req = Request(next_url, headers={"Accept": "application/json"})
                with urlopen(req, timeout=30) as resp:
                    data = json.loads(resp.read().decode())
            else:
                data = polygon_get("/v3/reference/tickers", params, api_key)

            for t in data.get("results", []):
                sym = t.get("ticker", "")
                sic = t.get("sic_code")
                if sym:
                    sector_map[sym] = sic_to_sector(sic)

            next_url = data.get("next_url")
            print(f"    Page {page}: {len(data.get('results', []))} tickers "
                  f"(total so far: {len(sector_map)})")
            if not next_url:
                break
    ...
    print(f"  [Polygon] Sector map built: {len(sector_map)} tickers")
    return sector_map
...
def main():
    ...
    # Fetch sector map from Polygon (build-time only — baked into baseline JSON)
    sector_map = fetch_polygon_sector_map(polygon_key)
