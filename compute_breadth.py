...
def main():
    ...
    for idx, date in enumerate(sorted_dates):
        ...
        for sym in components:
            ...
            # 52-week H/L: close-vs-close (standard definition)
            # today's close >= highest close of prior 252 trading days
            today_close = symbol_closes.get(sym, {}).get(date)
            if today_close is not None and idx >= 1:
                prior_start = max(0, idx - 252)
                prior_dates = sorted_dates[prior_start:idx]
                sym_closes_map = symbol_closes.get(sym, {})
                prior_high = max((sym_closes_map[d] for d in prior_dates if d in sym_closes_map), default=None)
                prior_low  = min((sym_closes_map[d] for d in prior_dates if d in sym_closes_map), default=None)
                if prior_high is not None and today_close >= prior_high:
                    new_highs += 1
                if prior_low is not None and today_close <= prior_low:
                    new_lows += 1

        _total = advances + declines
        net_ad = round((advances - declines) / _total * 1000, 4) if _total > 0 else 0.0
