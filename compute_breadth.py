...
def main():
    ...
    screener_data = {}  # sym -> { sma20, sma50, avgVol50, avgDolVol50 }
    ...
    for i in range(0, len(scr_symbols), BATCH_SIZE):
        ...
        if data.get("bars"):
            ...
            for sym, bars in data["bars"].items():
                ...
                rv = vols[-n_vol:]
                rp = closes[-n_vol:]
                avg_vol50     = round(sum(rv) / len(rv))
                avg_dolvol50  = round(sum(v * p for v, p in zip(rv, rp)) / len(rv))

                screener_data[sym] = {
                    "sma20":       sma20,
                    "sma50":       sma50,
                    "avgVol50":    avg_vol50,
                    "avgDolVol50": avg_dolvol50
                }
