...
def main():
    ...
    if not polygon_key:
        ...
        # Strategy: fetch Polygon grouped daily bars for each of the last 50 trading
        # days. Each call covers the entire market in one request, so we only need
        # 50 API calls total regardless of universe size. At 5 calls/minute (free
        # tier) this takes ~10 minutes — acceptable for a nightly job.
        #
        # We collect per-symbol arrays of (close, volume) then compute SMAs and
        # avg vol from those arrays.

        # Build list of the last 50 trading dates (Mon-Fri, excluding today)
