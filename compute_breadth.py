...
SCREENER_LOOKBACK_DAYS = 120  # Cal days of bars needed for SMA20/SMA50 + 50-day avg vol
POLYGON_RATE_LIMIT = 5       # Free tier: 5 requests/minute for aggregates
...
def polygon_grouped_daily(date_str, polygon_key):
    """Fetch Polygon grouped daily bars for all stocks on a given date.

    Returns a dict of { ticker: {"c": close, "v": volume} } for that day.
    Uses the /v2/aggs/grouped/locale/us/market/stocks/{date} endpoint which
    returns full SIP consolidated volume for every ticker in one request.
    """
    ...
    try:
        ...
        print(f"    Polygon HTTP {e.code} for {date_str}")
        return {}
    except (URLError, OSError) as e:
        print(f"    Polygon network error for {date_str}: {e}")
        return {}
    ...
    for bar in data.get("results") or []:
        t = bar.get("T")
        if t:
            result[t] = {"c": bar.get("c", 0), "v": bar.get("v", 0)}
    return result
...
def main():
    ...
    if not polygon_key:
        print("WARNING: POLYGON_API_KEY not set — screener volume will be skipped.")
    ...
    # --- Step 5: Screener data — SMA20, SMA50, 50-day avg vol (Polygon SIP volume) ---
    print(f"\n[5/5] Fetching screener universe and computing SMA/volume data...")
    ...
    scr_symbol_set = set(scr_symbols)

    screener_data = {}  # sym -> { sma20, sma50, avgVol50 }

    if not polygon_key:
        print("  Skipping volume fetch — POLYGON_API_KEY not set.")
    else:
        # Strategy: fetch Polygon grouped daily bars for each of the last 252 trading
        # days (≈1 trading year). Each call covers the entire market in one request.
        # - Last 50 bars → SMA20, SMA50, avgVol50
        # - All 252 bars → 52-week low (lowest close across the full year)
        # At 5 calls/minute (free tier) this takes ~50 minutes — acceptable for a
        # nightly job that runs unattended.

        # Build list of the last 252 trading dates (Mon-Fri, excluding today)
        today_utc = datetime.now(timezone.utc).date()
        trading_dates = []
        d = today_utc - timedelta(days=1)
        while len(trading_dates) < 252:
            if d.weekday() < 5:          # Mon-Fri only; Polygon skips holidays automatically
                trading_dates.append(d.strftime("%Y-%m-%d"))
            d -= timedelta(days=1)
        trading_dates.reverse()          # oldest → newest

        print(f"  Fetching {len(trading_dates)} trading days from Polygon "
              f"({trading_dates[0]} → {trading_dates[-1]})...")

        # per_sym: sym -> list of (close, volume) in chronological order
        per_sym = {}

        call_times = []   # track timestamps for rate limiting
        for idx, date_str in enumerate(trading_dates):
            # Rate limit: max POLYGON_RATE_LIMIT calls per 60 seconds
            call_times.append(time.monotonic())
            if len(call_times) > POLYGON_RATE_LIMIT:
                window_start = call_times[-POLYGON_RATE_LIMIT - 1]
                elapsed = time.monotonic() - window_start
                if elapsed < 60:
                    wait = 60 - elapsed + 0.2
                    print(f"    Rate limit pause {wait:.1f}s after call {idx + 1}/{len(trading_dates)}...")
                    time.sleep(wait)

            print(f"    [{idx + 1}/{len(trading_dates)}] {date_str}")
            day_bars = polygon_grouped_daily(date_str, polygon_key)

            for sym, bar in day_bars.items():
                if sym not in scr_symbol_set:
                    continue
                if sym not in per_sym:
                    per_sym[sym] = []
                per_sym[sym].append((bar["c"], bar["v"]))

        print(f"  Polygon fetch complete. Building screener metrics for {len(per_sym)} symbols...")

        for sym, cv_list in per_sym.items():
            if len(cv_list) < 20:
                continue
            closes = [c for c, v in cv_list]
            vols   = [v for c, v in cv_list]

            sma20 = round(sum(closes[-20:]) / 20, 4) if len(closes) >= 20 else None
            sma50 = round(sum(closes[-50:]) / 50, 4) if len(closes) >= 50 else None

            n_vol = min(50, len(vols))
            avg_vol50 = round(sum(vols[-n_vol:]) / n_vol)

            low52w = round(min(closes), 4)   # lowest close over all fetched bars (~1 year)
