...
def fetch_screener_universe():
    """Fetch current Russell 1000 + Russell 2000 constituents from SlickCharts.

    Tries to scrape /russell1000 and /russell2000 pages (same pattern as NDX-100).
    Falls back to FALLBACK_SCREENER_UNIVERSE if either scrape fails or returns
    too few symbols.
    ...
    sources = [
        ("Russell 1000", "https://www.slickcharts.com/russell1000", 800),
        ("Russell 2000", "https://www.slickcharts.com/russell2000", 1500),
    ]

    for name, url, min_count in sources:
        try:
            req = Request(url, headers=headers)
            with urlopen(req, timeout=20) as resp:
                html = resp.read().decode()
            syms = re.findall(r'/symbol/([A-Z]+)"', html)
            syms = [s for s in dict.fromkeys(syms) if '.' not in s]
            if len(syms) >= min_count:
                print(f"  Scraped {len(syms)} {name} components from SlickCharts")
                all_syms.extend(syms)
            else:
                print(f"  {name}: only {len(syms)} symbols scraped (expected >={min_count}) — using fallback")
                return _fallback_screener_universe()
        except Exception as e:
            print(f"  {name} scrape failed: {e} — using fallback")
            return _fallback_screener_universe()

    # Deduplicate preserving order, strip dot-tickers
    seen = set()
    result = []
    for s in all_syms:
    ...
    if len(result) < 2000:
        print(f"  Combined universe too small ({len(result)}) — using fallback")
        return _fallback_screener_universe()

    print(f"  Screener universe: {len(result)} unique symbols (live)")
    return result
...
def main():
    ...
    # Fetch live R1000+R2000 constituents; falls back to hardcoded list if scraping fails
    scr_symbols = fetch_screener_universe()
