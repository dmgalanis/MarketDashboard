...
def fetch_sector_map(finnhub_key, symbols, cached=None):
    """Fetch sector/industry for each symbol via Finnhub /stock/profile2.

    Only fetches symbols that are absent from `cached` (the sector map
    already written in the previous run of breadth_baseline.json).  On
    nightly runs this is typically 0-10 new symbols, keeping the action
    fast after the initial build.

    Respects the free-tier rate limit (FINNHUB_RATE_LIMIT req/min).
    Symbols that return no profile or an error are silently skipped.

    Returns a dict: { "AAPL": "Technology", "JPM": "Financial Services", ... }
    """
    # Start from whatever we already have
    sector_map = dict(cached) if cached else {}

    if not finnhub_key:
        print("  FINNHUB_KEY not set — skipping sector map (returning cached)")
        return sector_map

    # Only fetch symbols missing from the cache
    missing = [s for s in symbols if s not in sector_map]

    if not missing:
        print(f"  Sector map: all {len(sector_map)} symbols already cached — skipping Finnhub calls")
        return sector_map

    delay = 60.0 / FINNHUB_RATE_LIMIT  # seconds between requests
    print(f"  Sector map: {len(sector_map)} cached, {len(missing)} new symbols to fetch "
          f"(~{len(missing) * delay / 60:.1f} min)…")
    ...
    for i, sym in enumerate(missing, 1):
        ...
        time.sleep(delay)
        if i % 100 == 0:
            print(f"    {i} / {len(missing)} new symbols done")

    print(f"  Sector map complete: {len(sector_map)} total symbols mapped")
    return sector_map
...
def main():
    ...
    # Load cached sector map from the previous baseline (if it exists) so
    # we only hit Finnhub for symbols that are genuinely new.
    cached_sectors = {}
    if os.path.exists(OUTPUT_FILE):
        try:
            with open(OUTPUT_FILE) as f:
                prev = json.load(f)
            cached_sectors = prev.get("sectorMap") or {}
            print(f"  Loaded {len(cached_sectors)} cached sectors from previous baseline")
        except Exception as e:
            print(f"  Could not read previous baseline for sector cache: {e}")

    sector_map = fetch_sector_map(finnhub_key, scr_symbols, cached=cached_sectors)
