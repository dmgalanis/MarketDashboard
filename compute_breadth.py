...
Environment variables required:
    ALPACA_KEY_ID       — Alpaca API Key ID
    ALPACA_SECRET_KEY   — Alpaca Secret Key
    POLYGON_API_KEY     — Polygon.io API Key (for sector/industry data)
"""
...
# =============================================================================
# SIC CODE → READABLE SECTOR MAPPING
# =============================================================================

# Maps SIC code ranges to concise sector labels used in the screener UI.
# Based on SEC/OSHA SIC division structure, condensed to ~11 display sectors.
def sic_to_sector(sic_code):
    """Convert a numeric SIC code (int) to a short readable sector string."""
    if sic_code is None:
        return ""
    c = int(sic_code)
    if    100 <= c <=  999: return "Agriculture"
    if   1000 <= c <= 1499: return "Mining"
    if   1500 <= c <= 1799: return "Construction"
    if   2000 <= c <= 3999:
        # Manufacturing — break into meaningful sub-groups
        if 2000 <= c <= 2199: return "Food & Bev"
        if 2800 <= c <= 2899: return "Chemicals"
        if 2900 <= c <= 2999: return "Oil & Gas"
        if 3559 <= c <= 3679: return "Tech Hardware"
        if 3670 <= c <= 3679: return "Electronics"
        if 3812 <= c <= 3812: return "Defense"
        if 3821 <= c <= 3841: return "Med Devices"
        return "Industrials"
    if   4000 <= c <= 4999:
        if 4800 <= c <= 4899: return "Telecom"
        if 4900 <= c <= 4991: return "Utilities"
        return "Transportation"
    if   5000 <= c <= 5999: return "Retail/Trade"
    if   6000 <= c <= 6999:
        if 6000 <= c <= 6199: return "Banking"
        if 6200 <= c <= 6299: return "Securities"
        if 6300 <= c <= 6411: return "Insurance"
        if 6500 <= c <= 6599: return "Real Estate"
        if 6700 <= c <= 6799: return "Financials"
        return "Financials"
    if   7000 <= c <= 7999:
        if 7370 <= c <= 7379: return "Software"
        if 7389 <= c <= 7389: return "Services"
        return "Services"
    if   8000 <= c <= 8099: return "Healthcare"
    if   8700 <= c <= 8742: return "Consulting"
    if   8900 <= c <= 8999: return "Services"
    return "Other"
...
# NOTE: The bulk /v3/reference/tickers list endpoint does NOT return sic_code.
# sic_code is only available on the per-ticker detail endpoint:
#   GET /v3/reference/tickers/{ticker}
# We fetch each ticker individually using a thread pool for speed.

from concurrent.futures import ThreadPoolExecutor, as_completed

POLYGON_DETAIL_CONCURRENCY = 20   # parallel requests to Polygon detail endpoint
POLYGON_DETAIL_TIMEOUT     = 15   # seconds per request


def _fetch_one_polygon_ticker(sym, api_key):
    """Fetch /v3/reference/tickers/{sym} and return (sym, sector_str) or (sym, '')."""
    url = f"{POLYGON_BASE}/v3/reference/tickers/{sym}?apiKey={api_key}"
    try:
        req = Request(url, headers={"Accept": "application/json"})
        with urlopen(req, timeout=POLYGON_DETAIL_TIMEOUT) as resp:
            data = json.loads(resp.read().decode())
        result = data.get("results", {})
        sic = result.get("sic_code")
        return sym, sic_to_sector(sic)
    except Exception:
        return sym, ""


def fetch_polygon_sector_map(api_key, symbols=None):
    """Fetch per-ticker detail from Polygon for each symbol in `symbols` and
    return a dict mapping ticker -> sector string.

    Uses /v3/reference/tickers/{ticker} (the only endpoint that returns sic_code).
    Fetches concurrently via ThreadPoolExecutor for speed.
    Falls back to empty dict on total failure; individual misses silently return "".

    Args:
        api_key:  Polygon API key string.
        symbols:  List of ticker symbols to look up. If None, returns {}.
    """
    if not api_key:
        print("  [Polygon] No POLYGON_API_KEY - skipping sector fetch")
        return {}
    if not symbols:
        return {}
    ...
    print(f"  [Polygon] Fetching sector/SIC for {total} tickers "
          f"(concurrency={POLYGON_DETAIL_CONCURRENCY})...")

    try:
        with ThreadPoolExecutor(max_workers=POLYGON_DETAIL_CONCURRENCY) as pool:
            futures = {pool.submit(_fetch_one_polygon_ticker, sym, api_key): sym
                       for sym in symbols}
            for fut in as_completed(futures):
                ...
                if done % 100 == 0 or done == total:
                    print(f"    {done}/{total} tickers fetched")
    except Exception as e:
        print(f"  [Polygon] WARNING: sector fetch failed - {e}. Continuing without sectors.")
        return {}

    filled = sum(1 for v in sector_map.values() if v)
    print(f"  [Polygon] Sector map built: {filled}/{total} tickers with sector data")
    return sector_map
...
def main():
    ...
    # Fetch sector map from Polygon for the screener universe (build-time only — baked into baseline JSON)
    sector_map = fetch_polygon_sector_map(polygon_key, symbols=scr_symbols)
